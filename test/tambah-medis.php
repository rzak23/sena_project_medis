...
// => Cek ID Medis
$cari_id      = mysqli_query($koneksi, "SELECT id_medis FROM tabel_medis") or die(mysqli_error(""));
...
if ($data_id) {
  $id         = (int) $jumlah;
  $id         = $jumlah + 1;
  $id_medis   = "$tanggal" . "06" . str_pad($id, 7, "0", STR_PAD_LEFT);
} else {
...
if (isset($_POST["btn_simpan_medis"])) {
  if (tambah_medis($_POST) > 0) {
    //$_SESSION["idDokter"] = "namaDokter";
    echo "<script>alert ('Penambahan data rekam medis berhasil !'); document.location.href = 'rekam-medis.php';</script>";
  } else {
    echo "<script>alert ('Penambahan data rekam medis gagal !'); document.location.href = 'rekam-medis.php';</script>";
  }
}
            ...
            <form action="" method="post">
                ...
                <div class="col-md-4">
                  <div class="form-group row">
                    ...
                    <div class="col-md-9 input-group">
                      ...
                      <input type="text" name="biaya_medis" id="biaya_medis" class="form-control" required autocomplete="on" onkeydown="return numbersonly(this, event);" onkeyup="javascript:tandaPemisahTitik(this);">
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="row">
                <div class="col-md-12 text-right">
                  <button type="submit" name="btn_simpan_medis" id="btn_simpan_medis" onclick="simpanSession();" class="btn btn-success" style="width: 125px;">
...
<script type="text/javascript">
  ...
  if(sesdata != ""){
    document.getElementById('dokter_medis').value = sesdata;
  }
  var  sesparam = sessionStorage.getItem('paramku');
  if(sesparam != ""){
    document.getElementById('param_medis').value = sesparam;
  }
  var  sesjaga = sessionStorage.getItem('shiftku');
  if(sesjaga != ""){
    document.getElementById('shift_medis').value = sesjaga;
  }
  ...
  function simpanSession(){
    var dk = document.getElementById('dokter_medis').value;
    var pm = document.getElementById('param_medis').value;
    var sh = document.getElementById('shift_medis').value;
    sessionStorage.setItem('dokterku',dk);
    sessionStorage.setItem('paramku',pm);
    sessionStorage.setItem('shiftku',sh);
  }
